...
def empty_flow_data():
    ...
    return pd.DataFrame(columns=['date', 'net_flow', 'creation_units', 'redemption_units'])
...
def mock_data_manager():
    ...
    mock = Mock()
    mock.get_etf_flows = AsyncMock(return_value=pd.DataFrame({
        'date': pd.date_range(end=datetime.now(), periods=10, freq='D'),
        'net_flow': np.random.normal(0, 1000000, 10)
    }))
    mock.get_institutional_positioning = Mock(return_value={
        'institutional_ownership': 0.75,
        'net_buyer_count': 50,
        'total_institutions': 200,
        'avg_position_size': 5000000
