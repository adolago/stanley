...
class TestPiotroskiFScore:
    ...
    def test_fscore_individual_signals(self):
        ...
        assert (
            result.signals["roa_improving"] == True
        )  # Can't verify without prior year
...
class TestEarningsQualityAnalyzer:
    ...
    def test_aggregate_quality_rating(self):
        ...
        scores["m_score"] = MScoreResult(
            m_score=-1.5, manipulation_risk="High", components={}, flags=["High DSRI"]
        )
        scores["f_score"] = FScoreResult(
            ...
            signals={},
            positive_signals=[],
            negative_signals=["negative_roa"],
        )
        scores["z_score"] = ZScoreResult(
            z_score=1.5,
            distress_zone="Distress",
            bankruptcy_probability=0.70,
            components={},
        )
