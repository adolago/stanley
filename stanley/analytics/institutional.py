...
class InstitutionalAnalyzer:
    ...
    def analyze_13f_changes(self, manager_cik: str, 
                           quarter_over_quarter: bool = True) -> pd.DataFrame:
        """
    ...
    def _calculate_institutional_metrics(self, holdings: pd.DataFrame,
        ...
        if holdings.empty:
            return {
                ...
                'institution_count': 0,
                'top_holders': pd.DataFrame(columns=['manager_name', 'value_held', 'ownership_percentage']),
                'ownership_trend': 0.0,
        ...
        # Top holders
        top_holders = holdings.nlargest(5, 'value_held')[['manager_name', 'value_held', 'ownership_percentage']]

        # Ownership trend from recent changes
        recent_change = changes['net_institutional_change'].iloc[-1] if len(changes) > 0 else 0
        ownership_trend = np.sign(recent_change) * min(1, abs(recent_change) / 1000000)  # Normalize
        ...
        if n_institutions == 0:
            ...
            normalized_shares = ownership_shares / ownership_shares.sum() if ownership_shares.sum() > 0 else ownership_shares
            hhi = (normalized_shares ** 2).sum()
            # Normalize HHI to [0, 1] range
            min_hhi = 1.0 / n_institutions
            concentration_risk = (hhi - min_hhi) / (1.0 - min_hhi) if (1.0 - min_hhi) > 0 else 0.0
    ...
    def _calculate_13f_changes(self, current_filing: pd.DataFrame, 
                              previous_filing: pd.DataFrame) -> pd.DataFrame:
        """
